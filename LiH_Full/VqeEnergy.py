    ...
    for root, dirs, files in os.walk(base_dir):
        if "optimization_log.txt" in files:
            ...
            if energy is not None:
                ...
                # Extract numeric part of the folder name for sorting
                match = re.search(r"(\d+)", folder_name)
                folder_number = int(match.group(1)) if match else float('inf')  # Default to large number if no number
                energy_data.append((folder_name, folder_number, energy))
    return energy_data

# Function to save the energy data to a text file and plot it
def save_and_plot_energies(energy_data, output_txt, output_plot):
    # Sort by numeric part of folder names
    energy_data.sort(key=lambda x: x[1])

    # Save to text file
    with open(output_txt, 'w') as file:
        file.write("Folder\tEnergy\n")
        for folder, _, energy in energy_data:
            file.write(f"{folder}\t{energy}\n")

    # Plot the data
    folders = [x[0] for x in energy_data]
    energies = [x[2] for x in energy_data]
